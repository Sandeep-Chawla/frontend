...
function Section_two(props) {
  ...
  return (
    <div className='md:h-screen w-11/12 md:w-10/12 items-start justify-center relative mx-auto flex flex-col-reverse my-4'>
      <div className='md:h-full h-[90vh] w-full md:w-3/5'>
        {isLoading ? (
      ...
      <div className='bg-[#EBE8E8] flex items-center flex-col justify-center px-6 md:px-14 font-bold text-sm md:text-lg capitalize text-primary shadow-card md:w-1/2 h-[40vh] md:h-2/5 md:absolute right-0 z-10 leading-5'><div className='w-full lg:text-6xl text-4xl text-center left-0 top-10 uppercase bg-gradient-to-t from-[#837f7f42] from-25% -z-10 to-gray-600 bg-clip-text text-transparent py-2'>{props.title}</div>{props.text}</div>
        )}
    </div>
  );
}
